...
/**
 * Proxy endpoint for Peloton /api/workout/{workoutId}/performance_graph
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  try {
    ...
    // Parse query parameters (e.g., every_n)
    const url = new URL(request.url);
    const everyN = url.searchParams.get('every_n') || '1';
    
    // Forward the request to Peloton API with the same Authorization header
    const pelotonUrl = `${PELOTON_API_BASE}/api/workout/${workoutId}/performance_graph?every_n=${everyN}`;
